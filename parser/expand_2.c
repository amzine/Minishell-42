...
/*   Updated: 2022/11/29 21:41:02 by amimouni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (((str[ptr->i] == '$' && str[ptr->i + 1] == '"') || (str[ptr->i] == '$'
				&& str[ptr->i + 1] == '\'')) && (ptr->d_quot == 0))
		ptr->i++;
	else
	{
...
char	*comper_to_env(char *dest, char **env)
{
	...
	while (dst[i])
	{
		...
		{
			free(dest);
			dest = ft_substr(env[i], ft_strlen(dst[i]) + 1, ft_strlen(env[i])
					- ft_strlen(dst[i]));
			ft_free(dst);
			return (dest);
		}
