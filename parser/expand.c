...
/*   Updated: 2022/11/29 21:40:26 by amimouni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_quotes(t_token *ptr, char *str)
{
	...
	else if (str[ptr->n] == '\'' && quot_cont(str, ptr->n, '"') == 1
		&& !quot_cont(str, ptr->i, '\''))
		ptr->s_quot = 1;
}
...
char	*copy_no_expand(char *str, char *tmp, t_token *ptr, t_shell *mini)
{
	...
	if (str[ptr->i] == '$' && str[ptr->i + 1] == '?')
	{
		...
		tmp = ft_strrjoin(tmp, ft_substr(str, ptr->n
					+ ft_strlen(ft_itoa(mini->ret)) + 1, ptr->i - ptr->n
					- ft_strlen(ft_itoa(mini->ret)) - 1));
	}
	else if (ptr->s_quot == 1 || (((str[ptr->i] == '$' && str[ptr->i
						+ 1] == '\0') || (str[ptr->i] == '$' && str[ptr->i
						+ 1] == ' ')) || (ptr->d_quot == 1 && str[ptr->i] == '$'
				&& (str[ptr->i + 1] == '"' || str[ptr->i + 1] == '\''))))
	{
		tmp = copy(ptr, str, dest, tmp);
	}
	return (tmp);
}
...
char	*digit_expand(char *str, char *tmp, t_token *ptr, char **env)
{
	...
	{
		tmp = ft_strrjoin(tmp, ft_strdup("minishell"));
		ptr->i = cont_dollar(str, ptr->n + 1);
